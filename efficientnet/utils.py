...
def load_pretrained_weights(model, model_name, load_fc=True, advprop=True):
    """ Loads pretrained weights, and downloads if loading for the first time. """
    if advprop and int(model_name[-1]) == 8:
        path = 'weights/adv-{}-22a8fe65.pth'.format(model_name)
    if not advprop and int(model_name[-1]) == 0:
        path = "weights/efficientdet-d0.pth"
